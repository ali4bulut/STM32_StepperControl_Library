...
#define betaDef		0.604599787f		// 2 * proportional gain 0.604599787f
#define MAG_DEC		5.17f
...
volatile float rpy[3] = { 0.0f };
volatile float lin_acc[3] = { 0.0f };
//---------------------------------------------------------------------------------------------------
...
void update_rpy(float qw, float qx, float qy, float qz, float ax, float ay, float az) {
	...
	rpy[1] *= 180.0f / PI;
	rpy[2] *= 180.0f / PI;
	rpy[2] += MAG_DEC;	//Magnetic Declination: +5° 17'
	if (rpy[2] >= +180.f)
		rpy[2] -= 360.f;
	else if (rpy[2] < -180.f)
		rpy[2] += 360.f;
...
void no_filter(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz) {
	q0 += 0.5f * (-q1 * gx - q2 * gy - q3 * gz) * (1.0f / sampleFreq);
	q1 += 0.5f * (q0 * gx + q2 * gz - q3 * gy) * (1.0f / sampleFreq);
	q2 += 0.5f * (q0 * gy - q1 * gz + q3 * gx) * (1.0f / sampleFreq);
	q3 += 0.5f * (q0 * gz + q1 * gy - q2 * gx) * (1.0f / sampleFreq);
